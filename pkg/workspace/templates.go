...
// CopyPolicyTemplateFilesDryRun does a dry run of copying a policy template to a destination directory,
// to ensure it won't overwrite any files.
func (ppt PolicyPackTemplate) CopyPolicyTemplateFilesDryRun(destDir string) error {
	var existing []string
	if err := walkFiles(ppt.Dir, destDir, func(info os.FileInfo, source string, dest string) error {
		if destInfo, statErr := os.Stat(dest); statErr == nil && !destInfo.IsDir() {
			existing = append(existing, filepath.Base(dest))
		}
		return nil
...
// CopyPolicyPackTemplateFiles does the actual copy operation to a destination directory.
func (ppt PolicyPackTemplate) CopyPolicyPackTemplateFiles(
	destDir string, force bool, projectName string, projectDescription string) error {

	return walkFiles(ppt.Dir, destDir, func(info os.FileInfo, source string, dest string) error {
		if info.IsDir() {
			// Create the destination directory.
			return os.Mkdir(dest, 0700)
		}
